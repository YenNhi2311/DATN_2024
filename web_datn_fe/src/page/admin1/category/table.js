...
const TableCategory = () => {
    ...
    const fetchSkinTypes = async () => {
        try {
            const response = await apiClient.get("/api/categories"); // Gọi API
            const formattedData = response.data
                .map((item) => ({
                    id: item.categoryId, // Lưu ID loại da
    ...
    const handleOpenForm = (skinType = null) => {
        if (skinType) {
            ...
            setEditData({ categoryId: skinType.id, name: skinType.name });
        } else {
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) { // Nếu xác nhận xóa
            try {
                await apiClient.delete(`/api/categories/${id}`); // Gọi API để xóa
...
export default TableCategory; // Xuất component để sử dụng ở nơi khác
