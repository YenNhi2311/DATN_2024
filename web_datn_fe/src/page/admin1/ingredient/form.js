...
const FormIngredient = ({ onClose, editData }) => {
    // Khởi tạo state formData để lưu trữ thông tin form (ingredientId và name)
    const [formData, setFormData] = useState({ ingredientId: '', name: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.name) {
            setError('Tên thành phần là bắt buộc!');
            return;
        }

        // Nếu đang ở chế độ cập nhật, kiểm tra ID hợp lệ
        if (isUpdating && (!formData.ingredientId || isNaN(formData.ingredientId))) {
            setError('ID thành phần không hợp lệ!');
            return;
        }
        ...
        try {
            ...
            if (isUpdating) {
                // Gọi API PUT để cập nhật thành phần nếu đang trong chế độ cập nhật
                await axios.put(`http://localhost:8080/api/ingredients/${formData.ingredientId}`, dataToSend);
                Swal.fire({
                    title: "Thành công!",
                ...
                // Gọi API POST để thêm mới thành phần
                await axios.post('http://localhost:8080/api/ingredients', dataToSend);
                Swal.fire({
                    title: "Thành công!",
            ...
            console.error('Lỗi khi thêm/cập nhật thành phần:', error);
            Swal.fire({
                title: "Lỗi!",
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div className="row mb-4">
                <label htmlFor="name" className="col-sm-4 col-form-label">Tên thành phần: </label>
