...
const CategoryLists = () => {
  ...
  const filteredRows = rows.filter((row) => {
    return row.name.toLowerCase().includes(filters.name.toLowerCase());
  });
  ...
  return (
    <Box m="20px">
      <Header title="LOẠI SẢN PHẨM" subtitle="Quản lý loại sản phẩm" />
      ...
      <Dialog
        ...
        maxWidth="sm"
      >
        <DialogTitle>{editData ? "Chỉnh sửa loại" : "Thêm loại"}</DialogTitle>
        <DialogContent>
          <CategoryForm
            initialValues={editData || { name: "", place: "", img: "" }}
            onSubmit={(values) => {
              // Thực hiện thêm/sửa loại ở đây
              console.log(values);
              handleCloseForm();
