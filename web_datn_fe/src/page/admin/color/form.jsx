...
const ColorForm = ({ initialValues, onSubmit, handleClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = formValues.id
        ? await apiClient.put(`/api/colors/${formValues.id}`, formValues) // Cập nhật
        : await apiClient.post("/api/colors", formValues); // Thêm mới

      // Hiển thị thông báo thành công
      Swal.fire({
        title: "Thành công!",
        text: `Màu sắc đã được ${
      ...
      Swal.fire({
        title: "Lỗi!",
        text: "Có lỗi xảy ra khi lưu màu sắc. Vui lòng thử lại.",
        icon: "error",
        confirmButtonText: "OK",
      });
    } finally {
