  ...
  const fetchOrderCount = async () => {
    try {
      ...
      if (selectedPeriod === "today") {
        response = await apiClient.get("/api/revenues/canceled-count-today");
      } else if (selectedPeriod === "month") {
        response = await apiClient.get(
          "/api/revenues/canceled-count-by-month",
          {
            params: { month, year },
          }
        );
      } else if (selectedPeriod === "year") {
        response = await apiClient.get("/api/revenues/canceled-count-by-year", {
          params: { year },
        });
      }
