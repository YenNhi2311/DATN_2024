...
export default class LoaiSPShopTH extends Component {
  constructor(props) {
    super(props);
    this.state = {
      brands: [],
  ...
  getBrandIdFromURL = () => {
    ...
    return urlParts[urlParts.length - 1]; // Lấy ID thương hiệu từ cuối URL
  };
  ...
  fetchProducts = async () => {
    try {
      ...
      }, this.filterProductsByBrand); // Lọc theo brandId từ URL
    } catch (error) {
  ...
  filterProductsByBrand = () => {
    const { products, selectedBrandId } = this.state;

    if (selectedBrandId) {
      const filteredProducts = products.filter(
        (product) => product.brand && product.brand.brandId === Number(selectedBrandId)
      );
  ...
  handleSearch = (event) => {
    const query = this.removeAccents(event.target.value.toLowerCase().trim());
    this.setState({ searchQuery: query });

    if (query) {
      const filteredProducts = this.state.products.filter((product) => {
        const productName = this.removeAccents(product.name.toLowerCase());
  ...
  sortProducts = () => {
    ...
    this.setState({ filteredProducts: sortedProducts, currentPage: 1 });
  };
