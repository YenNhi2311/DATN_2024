...
const LoaiSPShop = () => {
  ...
  useEffect(() => {
    const categoryId = getCategoryIdFromURL(); // Lấy ID danh mục từ URL
    setSelectedCategoryId(categoryId); // Cập nhật trạng thái với ID danh mục đã chọn
    // Gọi các hàm fetch sau khi cập nhật trạng thái
    fetchBrands();
    fetchCategories();
    fetchSkins();
    fetchProducts();
  ...
  const filterProductsByCategory = (categoryId) => {
    if (categoryId) {
      const filtered = products.filter(
        (product) =>
          product.category && product.category.categoryId === Number(categoryId)
      );
      setFilteredProducts(filtered);
    } else {
      setFilteredProducts(products);
